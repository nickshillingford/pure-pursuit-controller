...
function calculatePerpendicularLine(pointA, pointD, road, lookAheadDistance) {
	const length = 15;
	const direction = new Vector3().subVectors(pointD, pointA).normalize();
	const perpendicular = (direction.x === 0 && direction.z === 0) ? new Vector3(1, 0, 0) : new Vector3(-direction.z, 0, direction.x).normalize();
	const lineLength = pointA.distanceTo(pointD);
	const pointB = new Vector3().addVectors(pointA, pointD).multiplyScalar(0.5);
	const displacement = direction.multiplyScalar(lookAheadDistance * lineLength);

	pointB.add(displacement);

	const perpPointA = new Vector3().addVectors(pointB, perpendicular.clone().multiplyScalar(length / 2));
	const perpPointB = new Vector3().subVectors(pointB, perpendicular.clone().multiplyScalar(length / 2));
	const line = new paper.Path(new paper.Point(perpPointA.x, perpPointA.z), new paper.Point(perpPointB.x, perpPointB.z));
	const intersections = road.center.getIntersections(line);
	const pointC = (intersections.length > 0) ? new Vector3(intersections[0]._point.x, 0.325, intersections[0]._point.y) : null;
